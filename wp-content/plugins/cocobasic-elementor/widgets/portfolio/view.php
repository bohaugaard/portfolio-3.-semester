...
if ($loop->have_posts()) :
    $return .= '<div class="portfolio-load-content-holder"></div><div class="grid" id="portfolio-grid"><div class="grid-sizer"></div>';
    while ($loop->have_posts()) : $loop->the_post();
        ...
        switch ($link_thumb_to):
            case 'link_to_image_url':
                ...
                $return .= '<div class="grid-item element-item ' . $p_size . ' ' . drop_cats_slug($post->ID) . '"><a class="item-link" href="' . $image_popup . '" data-rel="prettyPhoto[portfolio1]">';
                break;
            case 'link_to_video_url':
                $video_popup = get_post_meta($post->ID, "portfolio_video_popup", true);
                $return .= '<div class="grid-item element-item ' . $p_size . ' ' . drop_cats_slug($post->ID) . '"><a class="item-link" href="' . $video_popup . '" data-rel="prettyPhoto[portfolio1]">';
                break;
            case 'link_to_extern_url':
                $extern_site_url = get_post_meta($post->ID, "portfolio_extern_site_url", true);
                $return .= '<div class="grid-item element-item ' . $p_size . ' ' . drop_cats_slug($post->ID) . '"><a class="item-link" href="' . $extern_site_url . '" target="_blank">';
                break;

            default:
                $return .= '<div id="p-item-' . $post->ID . '" class="grid-item element-item ' . $p_size . ' ' . drop_cats_slug($post->ID) . '"><a class="item-link ajax-portfolio" href="' . get_permalink() . '" data-id="' . $post->ID . '">';
        endswitch;

        $return .= $portfolio_post_thumb . '<div class="portfolio-text-holder"><div class="portfolio-text-wrapper"><p class="portfolio-text">' . $p_thumb_title . '</p><p class="portfolio-cat">' . drop_cats_name($post->ID) . '</p></div></div></a></div>';
